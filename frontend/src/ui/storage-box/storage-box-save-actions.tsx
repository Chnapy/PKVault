...
> = ({ saveId, boxId, anchor }) => {
  ...
  const pkms =
    pkmSavePkmQuery.data?.data.filter((pkm) => ids.includes(pkm.id)) ?? [];
  ...
  const canEvolvePkms = pkms.filter((pkm) => {
    ...
    const evolveSpecies =
      staticEvolves?.trade[pkm.version] ??
      staticEvolves?.tradeWithItem[pkm.heldItemPokeapiName ?? ""]?.[
        pkm.version
      ];
          ...
          >
            ...
            {moveClickable.onClickAttached && (
              <ButtonWithDisabledPopover
                ...
                helpContent={t(
                  "storage.actions.move-attached-save.helpContent",
                )}
              >
                <Icon name="link" solid forButton />
                <Icon name="logout" solid forButton />
                {t("storage.actions.move-attached-save")} (
                {moveClickable.moveAttachedCount})
              </ButtonWithDisabledPopover>
            )}
