...
import { useStorageGetMainPkmVersions, useStorageGetSavePkms } from '../sdk/storage/storage.gen';
...
export const usePkmVersionSlotInfos = (baseVersionId: string | undefined) => {
    ...
    const baseVersion = baseVersionId !== undefined ? pkmVersionsData.find(version => version.id === baseVersionId) : undefined;
    const versions = baseVersion ? pkmVersionsData.filter(version => version.boxKey === baseVersion.boxKey) : [];
    ...
    const canRemoveAll = versions.length === 1 && mainVersion.canDelete;
