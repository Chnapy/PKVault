...
public class Dex5Service(SAV5 save) : DexGenService(save)
{
    ...
    public override async Task EnableSpeciesForm(ushort species, byte form, Gender gender, bool isSeen, bool isSeenShiny, bool isCaught, LanguageID[] languages)
    {
        ...
        if (isSeen)
        {
            ...
            save.Zukan.SetDisplayed(species, (byte)gender, false);
        }

        if (isSeenShiny)
        {
            save.Zukan.SetSeen(species, gender == Gender.Female ? 3 : 2, true);
            save.Zukan.SetDisplayed(species, (byte)gender, true);
        }
        ...
        if (formCount > 0)
        {
            if (isSeen)
            {
                ...
                save.Zukan.SetFormDisplayed(formIndex + form, false);
            }

            if (isSeenShiny)
            {
                save.Zukan.SetFormFlag(formIndex + form, 1, true);
                save.Zukan.SetFormDisplayed(formIndex + form, true);
            }
        }
