...
public class Dex5Service(SAV5 save) : DexGenService(save)
{
    ...
    public override async Task EnableSpeciesForm(EnableSpeciesFormPayload payload)
    {
        ...
        if (payload.IsSeen)
        {
            ...
            save.Zukan.SetDisplayed(payload.Species, (byte)payload.Gender, false);
        }

        if (payload.IsSeenShiny)
        {
            save.Zukan.SetSeen(payload.Species, payload.Gender == Gender.Female ? 3 : 2, true);
            save.Zukan.SetDisplayed(payload.Species, (byte)payload.Gender, true);
        }
        ...
        if (formCount > 0)
        {
            if (payload.IsSeen)
            {
                ...
                save.Zukan.SetFormDisplayed(formIndex + payload.Form, false);
            }

            if (payload.IsSeenShiny)
            {
                save.Zukan.SetFormFlag(formIndex + payload.Form, 1, true);
                save.Zukan.SetFormDisplayed(formIndex + payload.Form, true);
            }
        }
