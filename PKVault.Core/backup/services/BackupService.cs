...
)
{
    ...
    {
        using var _ = Log.Logger.Time("Create backup");

        var startTime = timeProvider.GetUtcNow().DateTime;

        var steptime = Log.Logger.Time($"Create backup - DB");
        var dbPaths = await CreateDbBackup();
        steptime.Dispose();

        steptime = Log.Logger.Time($"Create backup - Saves");
        var savesPaths = await CreateSavesBackup();
        steptime.Dispose();

        steptime = Log.Logger.Time($"Create backup - Storage");
        ...
        steptime = Log.Logger.Time($"Create backup - Compress");
        using (var memoryStream = new MemoryStream())
        {
            ...
            Log.Information($"Write backup to {bkpZipPath}");
        }
        steptime.Dispose();
    ...
    private async Task<Dictionary<string, (string TargetPath, byte[] FileContent)>> CreateMainBackup()
    {
        ...
        using var _ = Log.Logger.Time($"Prepare storage PKM files backup");
    ...
    {
        Log.Information($"Backup restore {createdAt}");

        var backup = GetBackup(createdAt);

        var logtime = Log.Logger.Time("Backup restore");
    ...
    public async Task PrepareBackupThenRun(string backupName, DataUpdateFlags flags, Func<Task> action)
    {
        ...
        try
        {
            var logtime = Log.Logger.Time("Action run with backup fallback");
        ...
        {
            Log.Error(ex, "Exception during action run");
