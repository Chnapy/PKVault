...
) : DataAction<DexSyncActionInput>
{
    protected override async Task<DataActionPayload> Execute(DexSyncActionInput input, DataUpdateFlags flags)
    {
        ...
        Log.Debug("\n\n");
        Log.Debug($"All values = {allValues.Count()}");
        Log.Debug($"Unique values = {uniqueValues.Count}");
        Log.Debug($"Filtered values = {filteredValues.Count()}");
        Log.Debug($"Grouped values = {groupedValues.Count()}");
        Log.Debug($"Ungrouped values = {ungroupedValues.Count()}");
        Log.Debug("\n\n");
