...
)
{
    ...
    public async Task<Dictionary<ushort, Dictionary<uint, DexItemDTO>>> GetDex(uint[] saveIds, HashSet<ushort>? speciesSet)
    {
        ...
        using var _ = Log.Logger.Time($"Update Dex with {saves.Count} saves (max-species={maxSpecies})");
    ...
    public DexGenService? GetDexService(SaveWrapper save)
    {
        ...
        {
            Log.Warning("Save version/gen not handled: " + save.Version + "/" + save.Generation);
            return null;
        }
