...
{
    protected override async Task<StaticSpeciesData> GetData(string[] rootParts)
    {
        ...
        for (ushort i = 1; i < (ushort)Species.MAX_COUNT; i++)
        {
            ...
            tasks.Add(Task.Run(async () =>
            {
                ...
                StaticSpeciesForm getVarietyForm(EntityContext context, Pokemon pkmObj, PokemonForm[] formObjs, int formIndex, StaticSpeciesForm? defaultForm)
                {
                    ...
                    {
                        Log.Logger.Error($"{formObj.Name} - ERROR FORM-NAMES {ex}");
                    }
                ...
                contexts.ForEach(context =>
                {
                    ...
                    (Pokemon, PokemonForm?, int)?[] formListData = [.. formListEn.Select((formNameEn, formIndex) =>
                    {
                        ...
                        (Pokemon, PokemonForm?, int)? searchFor (string name, bool intern) {
                            ...
                            if (result == null)
                            {
                                Log.Error($"FORM NOT FOUND for {defaultData.Item1.Name} // {context} // {formApiName} -> {string.Join(',', allDatas
    ...
    protected override string GetFilenameWithoutExtension() => StaticSpeciesLoader.GetFilenameWithoutExtension(lang);

    private static string GetLugiaShadowSprite()
    {
